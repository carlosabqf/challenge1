Feature: Test the 'GET' Method from '/courier'
  ...
  Scenario: Validate perform a /GET to Courier statement
    When I get statements to courier with id "1", start date "2023-12-27" and end date "2023-12-29"
    Then I validate that the code status is 200
    And I validate that the response contains the totalDeliveries: 10
    And I validate that the response contains the totalEarnings: 49.74

  Scenario: Validate perform a /POST to Courier statement, returning a 'Method Not Allowed' error.
    When I send a post call to statements to courier with id "1", start date "2023-12-27" and end date "2023-12-29"
    Then I validate that the code status is 405
    And I validate that the response contains the error message: "Method Not Allowed"